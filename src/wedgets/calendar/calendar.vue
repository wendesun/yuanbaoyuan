...
<template><div>
  <div @click.stop=""  class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
    <div  v-if="type!='time'">
      <div class="calendar-tools" onselectstart="return false">
                <span class="calendar-prev" @click="prev">
                     <img src="left.png" alt="上一个月"/>
                </span>
                <span class="calendar-next"  @click="next">
                     <img src="right.png" alt="下一个月"/>
                </span>
      ...
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (this.range) {
          ...
          // 没有默认值的时候显示选中今天日期
            ...
            if (this.begin != "") {
              if (beginTime < Number(new Date(chkY, chkM, i))) {
                temp[line].push({
                  day: i,
                  selected: true
                })
              ...
              if (endTime >  Number(new Date(chkY, chkM, i))) {
                temp[line].push({
                  day: i,
                  selected: true
                })
